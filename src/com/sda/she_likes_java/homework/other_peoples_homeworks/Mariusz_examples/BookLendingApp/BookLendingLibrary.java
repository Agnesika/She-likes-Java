package com.sda.she_likes_java.homework.other_peoples_homeworks.Mariusz_examples.BookLendingApp;

import java.time.Year;
import java.util.Random;public class BookLendingLibrary {
    public static void main(String[] args) {
        System.out.println("Let’s create some books");
        createBourneTrilogy();
        System.out.printf("Number of copies of all books: [%d]%n", Book.getNumberOfCreatedBooks());        Book[] createdArray = createBourneTrilogyAndStoreItInArray();
        ...
        System.out.printf("Number of copies of all books: [%d]%n", Book.getNumberOfCreatedBooks());    }    public static void createBourneTrilogy() {
        Random random = new Random();
        int numberOfIterations = random.nextInt();        for (int i = 0; i < numberOfIterations; i++) {
        ...
        booksShelf[1] = new Book(bookToCopy.getTitle(), bookToCopy.getAuthor(), bookToCopy.getYearOfPublication());        return booksShelf;
